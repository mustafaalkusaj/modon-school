...
function ColorSwatch({ color, label }: { color: string | null; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="h-8 w-8 rounded-lg border border-[var(--border)] shadow-sm" style={{ backgroundColor: bg }} />
      <span className="text-[9px] font-bold text-[var(--text-muted)]">{label}</span>
    </div>
  );
}

function ThemePresetCard({ preset }: { preset: BrandThemePreset }) {
  return (
    <div className="flex flex-col items-center gap-2 rounded-2xl border-2 border-[var(--border)] bg-[var(--surface-strong)] p-3 transition-all hover:scale-[1.03] hover:border-[var(--primary)]/30">
...
export function ThemesTab({ schools }: ThemesTabProps) {
  ...
  const filteredSchools = useMemo(() => {
    if (!search) return schools.filter((s) => !s.deleted_at);
    const q = search.toLowerCase();
    return schools.filter((s) => !s.deleted_at && s.name.toLowerCase().includes(q));
  }, [schools, search]);

  const filteredPresets = useMemo(() => {
    const allPresets: BrandThemePreset[] = families.flatMap((f) => f.presets);
    if (selectedFamily === "all") return allPresets;
    return allPresets.filter((p) => p.familyId === selectedFamily);
    ...
    <div className="space-y-6">
      <SectionCard title="ثيمات المدارس" description="معاينة ألوان كل مدرسة الحالية">
        <div className="mb-4">
          <div className="relative w-full max-w-md">
            ...
            <input
              className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] ps-10 pe-3 py-2 text-sm font-bold outline-none focus:ring-2 focus:ring-[var(--primary)]/20"
            ...
            {filteredSchools.map((school) => (
              <div key={school.id} className="group relative rounded-2xl border border-[var(--border)] bg-[var(--surface-strong)] p-4 transition hover:shadow-lg hover:border-[var(--primary)]/30">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button className="opacity-0 group-hover:opacity-100 transition rounded-lg p-1.5 text-[var(--text-muted)] hover:bg-[var(--surface-muted)] hover:text-[var(--primary)]" onClick={() => setPreviewSchool(school)}>
                    <Eye size={14} />
                  </button>
                </div>
                ...
                {!school.primary_color && (
                  <div className="mt-3 rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">لم يتم تخصيص الألوان</div>
                )}
              </div>
            ))}
          </div>
      ...
      <SectionCard title="معرض الثيمات" description={`${filteredPresets.length} ثيم متاح — ${families.length} عائلة لون`}>
        <div className="mb-4 flex flex-wrap gap-2">
          <button
            className={cx("rounded-xl px-3 py-1.5 text-xs font-black transition", selectedFamily === "all" ? "bg-[var(--primary)] text-white" : "bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:bg-[var(--border)]")}
            onClick={() => setSelectedFamily("all")}
          >
            الكل
          </button>
          {families.map((family) => {
            const firstPreset = family.presets[0];
            return (
              <button
                key={family.id}
                className={cx("rounded-xl px-3 py-1.5 text-xs font-black transition flex items-center gap-1.5", selectedFamily === family.id ? "bg-[var(--primary)] text-white" : "bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:bg-[var(--border)]")}
                onClick={() => setSelectedFamily(family.id)}
              >
                {firstPreset && <div className="h-3 w-3 rounded-full" style={{ backgroundColor: firstPreset.primaryColor }} />}
                {family.label}
              </button>
            );
          })}
          ...
          <div className="ui-dialog w-full max-w-lg overflow-hidden" role="dialog" aria-modal="true">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="rounded-2xl border border-[var(--border)] overflow-hidden">
                <div className="h-12 flex items-center px-4 gap-3" style={{ backgroundColor: previewSchool.primary_color || DEFAULT_SCHOOL_BRANDING.primary_color }}>
                  <div className="h-7 w-7 rounded-lg bg-white/20" />
                  <span className="text-sm font-black text-white">{previewSchool.name}</span>
                </div>
                <div className="flex">
                  <div className="w-16 min-h-[120px] p-2 space-y-2" style={{ backgroundColor: DEFAULT_SCHOOL_BRANDING.sidebar_color }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg border" style={{ backgroundColor: previewSchool.primary_color || DEFAULT_SCHOOL_BRANDING.primary_color }} />
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg border" style={{ backgroundColor: previewSchool.secondary_color || DEFAULT_SCHOOL_BRANDING.secondary_color }} />
