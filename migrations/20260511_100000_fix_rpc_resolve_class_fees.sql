-- Fix: school_payment_students_page RPC now resolves total_fee from class_fees
-- and computes remaining_fee correctly instead of using the raw DB generated column.
-- This prevents students from showing as "fully paid" when their student.total_fee
-- hasn't been populated but a matching class_fee exists.

-- Drop existing overloaded functions
DROP FUNCTION IF EXISTS public.school_payment_students_page(uuid, text, text, text, text, text, integer, integer);
DROP FUNCTION IF EXISTS public.school_payment_students_page(uuid, text, text, text, text, text, integer, integer, uuid[]);

-- Recreate: version WITH branch filtering (p_branch_ids)
...
AS $$
  ...
  filtered AS (
    SELECT
      ...
      s.phone,
      s.address,
      -- Resolve total_fee: prefer class_fees, fall back to student.total_fee
      COALESCE(cf.total_fee, s.total_fee, 0)::numeric AS total_fee,
      coalesce(s.paid_fee, 0)::numeric AS paid_fee,
      coalesce(s.discount_value, 0)::numeric AS discount_value,
      -- Compute remaining from resolved total, not from generated column
      GREATEST(
        COALESCE(cf.total_fee, s.total_fee, 0) - COALESCE(s.paid_fee, 0) - COALESCE(s.discount_value, 0),
        0
      )::numeric AS remaining_fee,
      s.status,
    ...
    WHERE s.school_id = p_school_id
      ...
      AND (
        ...
        OR (p.quick_filter = 'collected'
            AND GREATEST(
                  COALESCE(cf.total_fee, s.total_fee, 0) - COALESCE(s.paid_fee, 0) - COALESCE(s.discount_value, 0),
                  0
                ) <= 0
            AND COALESCE(cf.total_fee, s.total_fee, 0) > 0)
...
-- Also fix school_payments_summary to resolve from class_fees
DROP FUNCTION IF EXISTS public.school_payments_summary(uuid);

CREATE OR REPLACE FUNCTION public.school_payments_summary(
  p_school_id uuid
)
RETURNS TABLE(
...
AS $$
  WITH student_summary AS (
    SELECT
      count(*) FILTER (WHERE coalesce(s.status, 'active') NOT IN ('deleted', 'transferred'))::bigint AS total_students,
      coalesce(sum(COALESCE(cf.total_fee, s.total_fee, 0)) FILTER (WHERE coalesce(s.status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_fee,
      coalesce(sum(s.paid_fee) FILTER (WHERE coalesce(s.status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_paid,
      coalesce(sum(
        GREATEST(COALESCE(cf.total_fee, s.total_fee, 0) - COALESCE(s.paid_fee, 0) - COALESCE(s.discount_value, 0), 0)
      ) FILTER (WHERE coalesce(s.status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_remaining,
      count(*) FILTER (
        WHERE coalesce(s.status, 'active') NOT IN ('deleted', 'transferred')
          AND GREATEST(COALESCE(cf.total_fee, s.total_fee, 0) - COALESCE(s.paid_fee, 0) - COALESCE(s.discount_value, 0), 0) <= 0
          AND COALESCE(cf.total_fee, s.total_fee, 0) > 0
