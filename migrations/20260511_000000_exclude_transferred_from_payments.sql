-- Exclude transferred students from payments page summary and student list.
-- Transferred student revenue now only visible in reports page.
...
AS $$
  WITH student_summary AS (
    SELECT
      count(*) FILTER (WHERE coalesce(status, 'active') NOT IN ('deleted', 'transferred'))::bigint AS total_students,
      coalesce(sum(total_fee) FILTER (WHERE coalesce(status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_fee,
      coalesce(sum(paid_fee) FILTER (WHERE coalesce(status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_paid,
      coalesce(sum(remaining_fee) FILTER (WHERE coalesce(status, 'active') NOT IN ('deleted', 'transferred')), 0)::numeric AS total_remaining,
      count(*) FILTER (
        WHERE coalesce(status, 'active') NOT IN ('deleted', 'transferred')
...
-- Update the branch-aware version (9 params)
...
AS $$
  ...
  filtered AS (
    ...
    WHERE s.school_id = p_school_id
      ...
      AND (
        (p.quick_filter = 'deleted' AND s.status = 'deleted')
        OR (p.quick_filter = 'suspended' AND s.status = 'suspended')
        OR (p.quick_filter = 'graduated' AND s.status = 'graduated')
        OR (p.quick_filter = 'discounted' AND coalesce(s.discount_value, 0) > 0 AND coalesce(s.status, 'active') NOT IN ('deleted', 'transferred'))
        OR (p.quick_filter = 'collected' AND coalesce(s.remaining_fee, 0) <= 0 AND coalesce(s.total_fee, 0) > 0 AND coalesce(s.status, 'active') NOT IN ('deleted', 'transferred'))
