...
BEGIN
  IF to_regclass('public.students') IS NOT NULL THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_students_school_status_full_name ON public.students(school_id, status, full_name)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_students_school_class_name ON public.students(school_id, class_name)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_students_school_remaining_fee ON public.students(school_id, remaining_fee)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_students_school_total_fee ON public.students(school_id, total_fee)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_students_school_discount_value ON public.students(school_id, discount_value)';
  END IF;
END $$;
...
AS $$
  ...
  payment_summary AS (
    SELECT
      (SELECT count(*)::bigint FROM public.payments WHERE school_id = p_school_id) AS total_payment_count,
      coalesce(
        (
